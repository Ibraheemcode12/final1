...
// TO A POST EXCEPT THE POST OWNER.
    ...
    private final String folder = "C:/Users/Bashoofak/Desktop/ALL/PostServicee/src/main/java/com/example/PostServicee/images/";
    ...
    // so instead on the front end side we will let the user chose what post he wants and the system will load that one.
...
public ResponseEntity<?> getimage(String path){
    try{
    byte [] img = Files.readAllBytes(new File(folder+path).toPath());
    ...
    public boolean Add_post(String content,String username,MultipartFile file){
        try{
            ...
            post.setPath(path); // Saving the post image path.
...
if(list.size() ==0){ // no need to continue if this user has no posts to delete
return true;
}

        like_comment_feign.deletePosts(new deleteobj(list)); // Sending a list of post ids to the comment_like service to delete all the likes and comments reltaed to
                                                            // the users posts.
